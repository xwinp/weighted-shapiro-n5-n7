...
def resw_z(poly_in_v_w_z, label):
...
r1 = roots_in_01(R1,z); print("E1 w-double: %s"%["%.6f"%float(r) for r in r1]); all_roots|=set(r1)
# E2: v-double
...
r2 = roots_in_01(R2,z); print("E2 v-double: %s"%["%.6f"%float(r) for r in r2]); all_roots|=set(r2)
# E3: w=1
R3 = sp.expand(HC.subs(w,1))
r3 = roots_in_01(R3,z); print("E3 w=1: %s"%["%.6f"%float(r) for r in r3]); all_roots|=set(r3)
# E4: v=0
r4 = resw_z(E2u_num.subs(v,0), "E4 v=0"); all_roots|=set(r4)
# E5: v=1
r5 = resw_z(E2u_num.subs(v,1), "E5 v=1"); all_roots|=set(r5)
...
r6 = roots_in_01(R6,z); print("E6 u=1(v+w=1): %s"%["%.6f"%float(r) for r in r6]); all_roots|=set(r6)
# E7: a5=0 (numerator, with u=usol)
r7 = resw_z(Knum - Knum + sp.expand(sp.together(a5).as_numer_denom()[0])*0 + sp.expand(sp.together(a5).as_numer_denom()[0]), "E7 a5=0")
# simpler: a5_num = numerator of a5 (u=usol). build directly
a5_num = sp.expand(sp.together(a5).as_numer_denom()[0])
r7 = resw_z(a5_num, "E7 a5=0"); all_roots|=set(r7)
# E8: u-denom (1-w)-v^2=0
r8 = resw_z((1-w)-v**2, "E8 u-denom"); all_roots|=set(r8)
# E9: K_C=0
r9 = resw_z(Knum, "E9 K=0"); all_roots|=set(r9)

crit = sorted(float(r) for r in all_roots)
print("\nAll critical z in (0,1), sorted (%d):"%len(crit))
print(["%.6f"%c for c in crit])
with open('code/_hc_critical_z.pickle','wb') as f: pickle.dump(crit,f)
print("DONE-EVENTS")
