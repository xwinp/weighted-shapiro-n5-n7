...
- **$S_3$ — exact sign certificate.** With $r=(q/p)^{1/5}$, the KKT solution is $b=r^2,c=r^{-1},e=r^3,d=r(1-r^7)$; $d>0\iff r<1\iff p>1/2$. **Uniqueness:** the four Euler equations $\partial_bP=\partial_cP=\partial_dP=\partial_eP=0$ eliminate sequentially. Since $\partial_bP=\tfrac1{qc}-\tfrac1{pb^2}$ (independent of $d,e$ and strictly decreasing in $c>0$), $\partial_bP=0$ has the unique solution $c=pb^2/q=b^2/r^5$; thence $\partial_cP=\partial_dP=0$ give the unique pair $e=b^4/r^5$, $d=b^3(1-br^5)/r^5$; and the remaining equation factors exactly as
...
- $E_2^u=0$ (load-bearing). On $H_C=0$, $F_1$ and $F_2$ share a $v$-root — the KKT $v$-value — given exactly by the linear subresultant $S_1(F_1,F_2)=\sigma_1 v+\sigma_0$, i.e. $r=-\sigma_0/\sigma_1$. The **exact identity**
$$
\begin{aligned}
\sigma_1^{\,3}\,E_2^u(r) &= z\,(w-1)^2(z-1)^2\,B(w,z)\,A(w,z)\,H_C(w,z),\\
A &= -w^2z+wz^2-w-z^2+z,\qquad B=wz^2-wz-w-z^2+z,
\end{aligned}
$$
  holds with zero remainder. The subresultant coefficient is $\sigma_1=w\,g(w,z)$, and the resultant $\operatorname{Res}_w(g,H_C)=-z^{11}(z-1)^6$ vanishes only at the inadmissible endpoints $z=0,1$; for $z\in(0,1)$ the leading coefficients of $g,H_C$ in $w$ are nonzero, so this certifies $g\ne0$ at every real $w$-root of $H_C$. With $w\ne0$ (admissible), $\sigma_1\ne0$ on the admissible $H_C$ branch, so $H_C=0\Rightarrow E_2^u(r)=0$; admissibility ($v\ne1$) gives $E_2^{\rm red}=0$. (This replaces the earlier, incorrect claim that $F_1$ is $E_2^{\rm red}$ up to a prefactor: $F_1$ is not a scalar multiple of $E_2^u$ — `code/verify_s1_elim_step3.py` — but the two share the KKT $v$-root on $H_C=0$ by the identity above.)
